...
*Saved locally to workspace: business_content_ideas.md*`;
      ...
      for (const step of (planData.steps || [])) {
        ...
        if (previousStepOutput && typeof previousStepOutput === 'object') {
          if (!resolvedArgs.content && previousStepOutput.content) {
            resolvedArgs.content = previousStepOutput.content;
          }
...
Provide a fallback execution step to achieve the goal: "${userGoal}". Respond ONLY with JSON: { "action": "recovery action", "tool": "write_local_file" | "search_long_term_memory" | "query_document_rag", "args": {} }`;
